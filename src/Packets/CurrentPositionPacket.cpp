...
Uint32 CurrentPositionPacket::getX() {
    return SDLNet_Read32(&data[2]);
}

Uint32 CurrentPositionPacket::getY() {
    return SDLNet_Read32(&data[6]);
}

Uint32 CurrentPositionPacket::getTankRotation() {
    return SDLNet_Read32(&data[10]);
}

Uint32 CurrentPositionPacket::getTurretRotation() {
    return SDLNet_Read32(&data[14]);
}

void CurrentPositionPacket::print() const {
    std::cout << "CurrentPositionPacketReceived" << (int)getPlayerId() <<  std::endl;
}
